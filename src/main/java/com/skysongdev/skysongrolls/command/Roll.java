    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        for(int i = 0; i < opArgs.size(); i++){
            if(RollParser.checkValidOperator(opArgs.get(i))){
                commandSender.sendMessage(org.bukkit.ChatColor.DARK_GRAY + "[" + org.bukkit.ChatColor.GOLD + "SkysongRolls" + org.bukkit.ChatColor.DARK_GRAY + "] " + org.bukkit.ChatColor.RED + "Operator " + i + "(" + opArgs.get(i) + ") is invalid! (Correct form: <number>d<number>)");
                return true;
            }
        }
